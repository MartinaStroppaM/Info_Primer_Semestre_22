...
# La velocidad del auto se calcula así: _**(rpm / 100) * (0.5 + (cambio / 2))**_. 
# Por ejemplo en tercera a 2000 rpm, la velocidad es 20 * (0.5 + 1.5) = 40.
# También nos interesa controlar el consumo. Se parte de una base de 0.05 litros por kilómetro. 
# A este valor se le aplican los siguientes ajustes:
...
# Por ejemplo, si el motor está en primera y a 5000 rpm, entonces el consumo es 0.05 * 5 * 3 = 0.75 litros/km.
...
class Auto:
    ...
    def subirCambio(self): #Ver que no se vaya de los limites
        if self.cambio < 5:
            self.cambio += 1
    ...
    def subirRPM(self, cantidad):
        if self.rpm + cantidad <= 5000:
            ...
            raise Exception("El auto está por explotar") 

    def bajarRPM(self, cantidad): 
        if self.rpm - cantidad >= 0:
            self.rpm -= cantidad
        else:
            raise Exception("Se apagó el auto")
...
print("Velocidad del auto ", auto1.velocidad())
print("Consumo del auto ", auto1.ConsumoActualPorKm())
